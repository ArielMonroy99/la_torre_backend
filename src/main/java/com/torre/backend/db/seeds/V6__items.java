...
public class V6__items extends BaseJavaMigration {
  ...
  private static String getString(ArrayList<CreateItemDto> items) {
    StringJoiner valuesJoiner = new StringJoiner(",",
        "INSERT INTO item ( name, stock,minimum_stock, unit, type, category_id, created_at,created_by,status) VALUES ",
        ";");
    for (CreateItemDto createItemDto : items) {
      valuesJoiner.add(
          String.format("('%s','%s','%s', '%s','%s','%s',now(), 'migration', 'ACTIVE')",
              createItemDto.getName(), createItemDto.getStock(), createItemDto.getMinimumStock(),
              createItemDto.getUnit(), createItemDto.getType(), createItemDto.getCategoryId()));
    }
