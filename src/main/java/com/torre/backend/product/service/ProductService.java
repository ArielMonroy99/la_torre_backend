...
public class ProductService  {
    ...
    public void createProduct(CreateProductDto productDto, String username) {
        if(productDto == null) throw new BaseException(HttpStatus.BAD_REQUEST, "Errores de validación");
        Category category = categoryRepository.findById(productDto.getCategoryId()).orElseThrow(() -> new BaseException(HttpStatus.NOT_FOUND, "Categoria no encontrada"));
    ...
    public Page<ProductDto> getActivatedProducts(ProductQueryParamsDto queryParamsDto) {
        ...
        Page<Product> productPage = productRepository.filterActivatedProducts(filter, productName, buildPage(queryParamsDto));
    ...
    public Page<ProductDto> getProducts(ProductQueryParamsDto queryParamsDto) {
        String filter = "%" + queryParamsDto.getFilter() + "%";
        String productName = "%" + queryParamsDto.getProductName() + "%";
        Page<Product> productPage = productRepository.filterProducts(filter, productName, buildPage(queryParamsDto));
        log.info("page {}", productPage.getContent());
        return productPage.map(productMapper::toDto);
    }
    ...
    public void updateProduct(Long id, CreateProductDto productDto, String username) {
        if(productDto == null) throw new BaseException(HttpStatus.BAD_REQUEST, "Errores de validación");
        Category category = categoryRepository.findById(productDto.getCategoryId()).orElseThrow(() -> new BaseException(HttpStatus.NOT_FOUND, "Categoría no encontrada"));
        Product product = productRepository.findById(id).orElseThrow(() -> new BaseException(HttpStatus.NOT_FOUND, "Producto no encontrado"));
    ...
    public void updateStatus(Long id, String username, StatusEnum statusEnum) {
        ...
        if (product == null) throw new BaseException(HttpStatus.NOT_FOUND,"Category not found");
